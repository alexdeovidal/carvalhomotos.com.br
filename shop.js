...
function cardMarkup(product) {
  ...
  return `<article class="product-card">
    ...
    <div class="product-info"><p class="product-type">${product.type}</p><h3>${product.name}</h3><p class="product-price">Preço sob consulta • Chame e simule</p><div class="product-actions"><a class="product-buy-button" href="${buyUrl}" target="_blank" rel="noopener noreferrer" aria-label="Comprar ${product.name} pelo WhatsApp">Comprar pelo WhatsApp ${ARROW_NE}</a><button type="button" class="product-add-button" data-add="${product.id}" aria-label="Adicionar ${product.name} à lista">+ Lista</button></div><button type="button" class="product-detail-button" data-detail="${product.id}">Ver detalhes e ficha ${ARROW_NE}</button></div>
  </article>`;
}
